...
function renderRichTextAsHtml(richTextArray = []) {
...
export function generateRSS(posts) {
  const feed = new RSS({
    title: "Boris Kirov",
    description: "Daynotes and Third Place by Boris Kirov.",
  ...
  posts.forEach((post) => {
    ...
    const contentBlocks =
      ...
      [];
    const fullContent = renderRichTextAsHtml(contentBlocks).trim();
